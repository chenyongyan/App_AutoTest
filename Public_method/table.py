...
def cretaTable(self):
        ...
        workbook.save('E:\\App_AutoTest\\Public_file\\book.xlsx')


# 向表格继续添加数据
def write(self,rows,value):
        excel_path='E:\\App_AutoTest\\Public_file\\book.xlsx'
...
# 读取表格数据
def read(self,rows):
        wb = xlrd.open_workbook(u'E:\\App_AutoTest\\Public_file\\book.xlsx','w+b')
...
def wipe_excel(self):
        try:
                os.remove('E:\\App_AutoTest\\Public_file\\book.xlsx')
                print('数据删除成功！')
        except:
                print('数据清理失败！')
